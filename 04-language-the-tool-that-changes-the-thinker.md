...
In the previous chapters we saw notations that let us count and tables that let us reason. Language is the same kind of thing, only more dangerous. It does not merely describe our thoughts. It changes the thoughts we can comfortably have.

> **Interactive exhibit placeholder: `words-change-the-problem`**
>
> Present the same tiny system using three different vocabularies: machine words, business words, and visual metaphor. Let the reader rename things and see which relationships become easier or harder to notice. The point is that naming is not decoration; it is a design decision.
